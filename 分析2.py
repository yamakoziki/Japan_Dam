...
def write_s8(wb, dams, glossary):
    ...
    # ── サマリー ──
    all_syms = [r["symbol"] for d in hokdev_sorted for r in d["recs"]]
    p1_list  = [sym_parts(s)[0] for s in all_syms]
    p2_list  = [sym_parts(s)[1] for s in all_syms]
    b_scores = [b_score(r["bearing_cap"]) for d in hokdev_sorted
                for r in d["recs"] if b_score(r["bearing_cap"])]
    p_scores = [p_score(r["permeability"]) for d in hokdev_sorted
                for r in d["recs"] if p_score(r["permeability"])]
    risks    = [risk_rank(r["bearing_cap"], r["permeability"])
                for d in hokdev_sorted for r in d["recs"]]

    summary = [
        ("対象ダム数（北海道開発局）",          len(hokdev_all)),
        ("うちデータあり",                       len(hokdev)),
        ("ユニークSymbol数（3項目）",            len(set(all_syms))),
        ("ユニーク1項目数",                      len(set(p1_list))),
        ("ユニーク2項目数",                      len(set(p2_list))),
        ("平均強度スコア(1-5)",                  avg(b_scores)),
        ("平均透水性スコア(1-4)",                avg(p_scores)),
        ("平均リスクスコア",                     avg(risks)),
        ("強度低（スコア≤2）ダム層数(%)",        pct(sum(1 for s in b_scores if s<=2), len(b_scores))),
        ("透水性高（スコア≥4）ダム層数(%)",      pct(sum(1 for s in p_scores if s>=4), len(p_scores))),
    ]
    ...
    for label, val in summary:
        ...
        ws.cell(row, 2, val).font  = BODY_FONT
        ws.cell(row, 2).alignment  = RIGHT
        row += 1
    row += 1
    ...
    hdrs = ["ダム名","堤高(m)","所在地","Symbol-1","1項目-1","2項目-1",
            "Symbol-2","Symbol-3","Symbol-4","Symbol-5",
            "平均強度","平均透水性","平均リスク","主リスク"]
    widths = [16, 8, 22, 18, 12, 16, 16, 16, 16, 16, 10, 10, 10, 22]
    for ci, (h, w) in enumerate(zip(hdrs, widths), 1):
        hdr(ws, row, ci, h, w)
    row += 1
    ...
    for d in hokdev_sorted:
        ...
        bs   = [b_score(r["bearing_cap"]) for r in d["recs"] if b_score(r["bearing_cap"])]
        ps   = [p_score(r["permeability"]) for r in d["recs"] if p_score(r["permeability"])]
        rks  = [risk_rank(r["bearing_cap"], r["permeability"]) for r in d["recs"]]
        risk_top = Counter(r["main_risk"] for r in d["recs"]).most_common(1)
        r_avg = avg(rks)
        fill = RED_FILL if isinstance(r_avg, float) and r_avg >= 6 else \
               YEL_FILL if isinstance(r_avg, float) and r_avg >= 4 else \
               ALT_FILL if row % 2 == 0 else None
        s1 = syms[0] if syms else ""
        vals = [d["name"], d["height"], d["loc"][:22] if d["loc"] else "",
                s1, sym_parts(s1)[0] if s1 else "", sym_parts(s1)[1] if s1 else "",
                syms[1] if len(syms) > 1 else "",
                syms[2] if len(syms) > 2 else "",
                syms[3] if len(syms) > 3 else "",
    ...
    # ── Symbol頻度（1項目・2項目・3項目）──
    sec_title(ws, row, 1, 8, "▼ Symbol頻度（1項目・2項目・3項目）")
    row += 1
    for ci, t in enumerate(["1項目","件数","2項目","件数","3項目フル","件数","強度avg","透水性avg"], 1):
        sub(ws, row, ci, t, 18 if ci in (1, 3, 5) else 8)
    row += 1
    ...
    p1c = Counter(p1_list)
    p2c = Counter(p2_list)
    p3c = Counter(all_syms)
    p1l = p1c.most_common()
    p2l = p2c.most_common()
    p3l = p3c.most_common()
    for i in range(max(len(p1l), len(p2l), len(p3l))):
        fill = ALT_FILL if row % 2 == 0 else None
        v1 = p1l[i] if i < len(p1l) else ("", "")
        v2 = p2l[i] if i < len(p2l) else ("", "")
        v3 = p3l[i] if i < len(p3l) else ("", "")
        if v3[0]:
            rec = next((r for r in glossary.values() if r["symbol"] == v3[0]), {})
            b_a = b_score(rec.get("bearing_cap", ""))
            p_a = p_score(rec.get("permeability", ""))
        else:
            b_a = p_a = ""
        for ci, v in enumerate([v1[0], v1[1], v2[0], v2[1], v3[0], v3[1], b_a, p_a], 1):
            al = RIGHT if ci in (2, 4, 6, 7, 8) else LEFT
    ...
    def risk_high_pct(sym_set):
        recs = [r for r in glossary.values() if r["symbol"] in sym_set]
        cnt  = sum(1 for r in recs if risk_rank(r.get("bearing_cap",""), r.get("permeability","")) >= 6)
    ...
    for label, cnt, rh, note in cmp_rows:
        fill = ALT_FILL if row % 2 == 0 else None
        for ci, v in enumerate([label, cnt, rh, note], 1):
            al = RIGHT if ci in (2, 3) else LEFT
            body(ws, row, ci, v, align=al, fill=fill)
        row += 1
    ...
    if only_dev:
        ...
        row += 1
        all_sym_cnt = Counter(r["symbol"] for d in dams for r in d["recs"])
        for sym in sorted(only_dev, key=lambda s: -all_sym_cnt.get(s, 0)):
            rec = next((r for r in glossary.values() if r["symbol"] == sym), {})
            rr  = risk_rank(rec.get("bearing_cap", ""), rec.get("permeability", ""))
            fill = RED_FILL if isinstance(rr, int) and rr >= 6 else \
                   YEL_FILL if isinstance(rr, int) and rr >= 4 else \
                   ALT_FILL if row % 2 == 0 else None
            vals = [sym, sym_parts(sym)[0], sym_parts(sym)[1],
                    rec.get("geo_era", ""), rec.get("geo_rock", ""),
                    all_sym_cnt.get(sym, 0),
                    rec.get("bearing_cap", ""), rec.get("permeability", ""), rr]
            for ci, v in enumerate(vals, 1):
                al = RIGHT if ci in (6, 9) else LEFT
                body(ws, row, ci, v, align=al, fill=fill)
            row += 1
